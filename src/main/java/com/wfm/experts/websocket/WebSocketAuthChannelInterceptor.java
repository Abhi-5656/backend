...
public class WebSocketAuthChannelInterceptor implements ChannelInterceptor {
    ...
    private void handleConnect(StompHeaderAccessor accessor) {
        try {
            ...
            String email = jwtUtil.extractEmail(token);
            String tenantId = jwtUtil.extractTenantId(token);
            String employeeId = jwtUtil.extractEmployeeId(token); // Extract the employeeId
            ...
            // CRITICAL CHANGE HERE: Use employeeId as the principal for the WebSocket session
            UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
                    employeeId, null, userDetails.getAuthorities());
            ...
            logger.info("STOMP CONNECT :: Authenticated '{}' for tenant '{}'", employeeId, tenantId);
