...
public class StandardHoursRule implements PayPolicyRule {
    ...
    public PayPolicyRuleResultDTO execute(PayPolicyExecutionContext context) {
        ...
        // **MODIFIED LOGIC**: Prioritize AFTER_FIXED_HOURS trigger regardless of schedule.
        if (payPolicy != null && payPolicy.getOvertimeRules() != null && payPolicy.getOvertimeRules().getDailyOtTrigger() == DailyOtTrigger.AFTER_FIXED_HOURS) {
            OvertimeRules otRules = payPolicy.getOvertimeRules();
            int dailyThreshold = (otRules.getThresholdHours() != null ? otRules.getThresholdHours() * 60 : 0) + (otRules.getThresholdMinutes() != null ? otRules.getThresholdMinutes() : 0);

            int netThreshold = dailyThreshold - unpaidBreakMinutes;
            standardMinutes = Math.min(workedMinutes, netThreshold);
            minutesBeyondStandard = Math.max(0, workedMinutes - standardMinutes);

            // Scheduled Employee Logic (runs only if AFTER_FIXED_HOURS is not the trigger)
        } else if (employeeShift != null && employeeShift.getShift() != null) {
            long shiftDuration = Duration.between(employeeShift.getShift().getStartTime(), employeeShift.getShift().getEndTime()).toMinutes();
            if (shiftDuration < 0) {
                shiftDuration += 1440; // Handle overnight shift
            }
            standardMinutes = Math.min(workedMinutes, (int) shiftDuration - unpaidBreakMinutes);
            minutesBeyondStandard = Math.max(0, workedMinutes - standardMinutes);
