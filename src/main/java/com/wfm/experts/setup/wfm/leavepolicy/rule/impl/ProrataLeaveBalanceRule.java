...
// Save as: src/main/java/com/wfm/experts/setup/wfm/leavepolicy/rule/impl/ProrataLeaveBalanceRule.java
package com.wfm.experts.setup.wfm.leavepolicy.rule.impl;
...
public class ProrataLeaveBalanceRule implements LeavePolicyRule {
    ...
    public LeavePolicyRuleResultDTO execute(LeavePolicyExecutionContext context) {
        ...
        LocalDate hireDate = employee.getOrganizationalInfo().getEmploymentDetails().getDateOfJoining();
        double totalLeaves = fixedGrant.getRepeatedlyDetails().getMaxDaysPerYear() != null ?
                fixedGrant.getRepeatedlyDetails().getMaxDaysPerYear() :
                (fixedGrant.getRepeatedlyDetails().getMaxDaysPerMonth() != null ?
                        fixedGrant.getRepeatedlyDetails().getMaxDaysPerMonth() * 12 : 0);
