...
public class PunchEventRules implements PayPolicyRule {
    ...
    public PayPolicyRuleResultDTO execute(PayPolicyExecutionContext context) {
        ...
        if (shift == null || shift.getShift() == null || shift.getShift().getStartTime() == null || shift.getShift().getEndTime() == null) {
            return PayPolicyRuleResultDTO.builder()
                    .ruleName(getName())
                    .result("NO_SHIFT")
                    .success(false)
                    .message("No valid scheduled shift found for this day.")
                    .build();
        }
        ...
        punches.stream()
                ...
                .ifPresent(inPunch -> {
                    LocalTime punchTime = inPunch.getEventTime().toLocalTime();
                    long diff = Duration.between(shiftStartTime, punchTime).toMinutes();

                    if (diff > 0 && lateIn != null && diff > lateIn) {
                        violations.append(String.format("Late In: %d mins late. ", diff));
                    } else if (diff < 0 && earlyIn != null && -diff > earlyIn) {
                        violations.append(String.format("Early In: %d mins early. ", -diff));
                    }
                });
        ...
        punches.stream()
                ...
                .ifPresent(outPunch -> {
                    LocalTime punchTime = outPunch.getEventTime().toLocalTime();
                    long diff = Duration.between(shiftEndTime, punchTime).toMinutes();

                    if (diff > 0 && lateOut != null && diff > lateOut) {
                        violations.append(String.format("Late Out: %d mins late. ", diff));
                    } else if (diff < 0 && earlyOut != null && -diff > earlyOut) {
                        violations.append(String.format("Early Out: %d mins early. ", -diff));
                    }
                });

        String result = violations.length() > 0 ? "VIOLATION" : "OK";

        return PayPolicyRuleResultDTO.builder()
                .ruleName(getName())
                .result(result)
                .success(true)
                .message(violations.length() > 0 ? violations.toString().trim() : "All punches are within grace periods.")
                .build();
    }
}
