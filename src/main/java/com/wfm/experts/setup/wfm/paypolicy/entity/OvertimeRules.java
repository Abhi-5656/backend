...
public class OvertimeRules implements PayPolicyRule {
    ...
    private void executeScheduledOt(PayPolicyExecutionContext context) {
        ...
        // Get the grace period from the rules.
        int gracePeriod = gracePeriodAfterShiftEnd != null ? gracePeriodAfterShiftEnd : 0;

        // Trigger OT only if the employee works beyond the grace period.
        if (minutesForOtCalculation > gracePeriod) {
            // If triggered, the entire duration after shift end is considered OT.
            dailyOt = minutesForOtCalculation;
            ...
            // If within the grace period, move the minutes to excess hours.
            excessOt = minutesForOtCalculation;
        }
