...
public class LeaveBalanceRecalculationConsumer {
    ...
    public void handleLeaveBalanceRecalculationRequest(
        ...
        try {
            logger.info("Processing leave balance INCREMENT for employee: '{}', Month: '{}', Tenant: '{}'",
                    request.getEmployeeId(), YearMonth.from(request.getWorkDate()), tenantIdFromHeader);

            // --- THIS IS THE CRITICAL CHANGE ---
            // Instead of recalculating all time, we just increment for the month of the punch.
            // We pass the YearMonth of the punch that triggered this.
            leaveAccrualService.incrementEarnedGrantForMonth(
                    request.getEmployeeId(),
                    YearMonth.from(request.getWorkDate())
            );
            // ------------------------------------

            logger.info("Successfully processed leave balance increment for employee: '{}', Tenant: '{}'",
                    request.getEmployeeId(), tenantIdFromHeader);

        } catch (Exception e) {
            logger.error("Unexpected error processing leave balance increment for employee: '{}', Tenant: '{}'. Error: {}. Rejecting message.",
                    request.getEmployeeId(), tenantIdFromHeader, e.getMessage(), e);
            throw new AmqpRejectAndDontRequeueException("Unexpected error processing leave balance increment request: " + e.getMessage(), e);
        } finally {
            TenantContext.clear();
            logger.debug("TenantContext cleared after processing leave balance increment for employee: {}", request.getEmployeeId());
        }
    }
}