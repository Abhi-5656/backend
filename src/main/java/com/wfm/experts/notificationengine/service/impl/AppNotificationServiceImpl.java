...
public class AppNotificationServiceImpl implements AppNotificationService {
    ...
    //    @Override
    @Transactional
    public int markPendingApprovalAsRead(Long approvalId, String userId) {
        String currentTenantId = TenantContext.getTenant();
        logger.info("Attempting to mark pending notification for approvalId: {} as read for userId: {} in tenant: {}",
                approvalId, userId, currentTenantId);

        int updatedCount = appNotificationRepository.markPendingApprovalAsReadByApprovalId(
                approvalId,
                userId,
                LocalDateTime.now()
        );
        logger.info("Marked {} pending approval notifications for approvalId: {} as read for userId: {} in tenant: {}",
                updatedCount, approvalId, userId, currentTenantId);

        // Optionally send a WebSocket update to notify the user that an item has been cleared
        if (updatedCount > 0) {
            String userPrincipalName = userId;
            String destination = "/queue/in-app-notifications";
            // Sending a small object or a simple count update
            messagingTemplate.convertAndSendToUser(userPrincipalName, destination, Map.of("action", "notification_cleared", "approvalId", approvalId));
        }

        return updatedCount;
    }
    /**
     * Updates the 'leaveStatus' field within the additionalData JSONB column for a specific notification.
     */
    @Transactional
    public int updateLeaveStatusOfNotificationByApprovalId(Long approvalId, String userId, String newLeaveStatus) {
        return appNotificationRepository.updateLeaveStatusInAdditionalData(approvalId, userId, newLeaveStatus);
    }
}
